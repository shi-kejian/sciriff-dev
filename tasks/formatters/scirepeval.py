    ...
    def format_instance(self, instance):
        ...
        for candidate in instance["candidates"]:
            if "abstract" in candidate:
                candidate["abstract"] = candidate["abstract"][: self.abstract_character_limit]
        ...
        candidates_dicts = [OrderedDict((k, candidate[k]) for k in self.fields_to_keep) for candidate in candidates]
        # actual string to output
        candidates_string = "\n".join([json.dumps(candidate_dict, indent=2) for candidate_dict in candidates_dicts])

        # ground truth of rank ordering is based on the score field
        ranking_tuples = [(candidate["id"], candidate["score"]) for candidate in candidates]
        # sort by score
...
if __name__ == "__main__":
    ...
    instances = datasets.load_dataset("allenai/scirepeval", "trec_covid", "evaluation")
