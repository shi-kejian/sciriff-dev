...
class Task(yaml.YAMLObject):
    ...
    def __init__(
        ...
        # Make sure there's as most 1 evaluation template.
        self._check_eval_templates()
    ...
    def _check_eval_templates(self):
        eval_templates = [t for t in self.templates.values() if t.evaluate is True]
        if len(eval_templates) > 1:
            raise ValueError(f"Task {self.name} has more than one evaluation template.")
    ...
    def get_split_names(self):
        ...
        if self.data_dir is not None:
            data_path = f"{paths.DATA_DIR}/processed/{self.data_dir}"
        else:
            data_path = self.dataset

        if self.subset is None:
            splits = datasets.get_dataset_split_names(data_path)
        else:
            splits = datasets.get_dataset_split_names(data_path, self.subset)
    ...
    def apply(
        ...
        if self.data_dir is not None:
            ...
            instances = datasets.load_dataset(
                "json",
                data_dir=f"{paths.DATA_DIR}/processed/{self.data_dir}",
                split=split,
            )
        else:
            # Otherwise load from Huggingface.
            instances = datasets.load_dataset(self.dataset, self.subset, split=split)
        ...
        if self.formatter is not None:
            formatter_module = importlib.import_module(
                f"tasks.formatters.{self.formatter['name']}"
            )
        ...
        if split == "test":
            # Get the evaluation template id.
            template_ids = [
                template._id
                for template in self.templates.values()
                if template.evaluate is True
            ]
            if len(template_ids) == 0:
                raise Exception("No evaluation template found.")
