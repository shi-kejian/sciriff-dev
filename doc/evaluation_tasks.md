...
- Task Input: A citation sentence from a research apper.
...
- Metrics: Automated metrics are unreliable, so we use three. (1) Rouge. (2) GPT-3.5 to judge similarity of generated summary to human reference on 1-5 scale. (3) GPT-4 to compare quality of generated summary vs. summary generated by a baseline model -- in our case, a version of Tulu trained with no science data.
...
- Metrics: We compute metrics for both the answer and the evidence.
  - Answer: (1) Token-level F1. (2) GPT-3.5 judge of similarity of model answer to human reference (1-5 scale).
  - Evidence: Token F1.
...
- Metrics: We use label F1 for the verdict, and sentence F1 for the evidence.
