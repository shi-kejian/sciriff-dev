...
def make_parser():
    ...
    parser.add_argument(
        "--tulu_format",
        action="store_true",
        help="If given, format inputs for Tulu models.",
    )
    parser.add_argument(
        "--tasks",
    ...
    parser.add_argument(
        "--interactive",
        action="store_true",
        help="""If given, run this job within the current interactive session instead
        of kicking off a batch job.""",
    )
    ####################
    # All arguments below this point only apply to Beaker batch jobs; ignore otherwise.
    parser.add_argument(
        "--beaker_cluster",
        type=str,
        default=None,
...
def make_task_command(task_name, result_dir, args):
    ...
    if args.tulu_format:
        include_path = paths.EVAL_DIR / "eleuther_templates/tulu"
    else:
        include_path = paths.EVAL_DIR / "eleuther_templates/general"
