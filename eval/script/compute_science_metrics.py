...
from eval import tasks as eval_tasks
from eval.tasks._base import EvalTask
...
class ScienceEvaluator:
    ...
    def make_summary_table(self, metrics_summary):
        ...
        pvt["mean"] = pvt[summary_cols].mean(axis=1)
        pvt["median"] = pvt[summary_cols].median(axis=1)
        pvt.index.name = None
        ix_sorted = sorted(pvt.index, key=sort_key)
        pvt = pvt.loc[ix_sorted]
        pvt.to_csv(
    ...
    def eval_one_model(self, model_dir):
        ...
        for pred_dir in model_dir.iterdir():
            ...
            # Directory with predictions of baseline model. Right now, we're just doing
            # baseline-based predictions for MUP since the human summaries aren't that
            # good.
            baseline_dir = self.pred_dir / self.baseline_model / pred_dir.name
            ...
            evaluator = eval_tasks.registry.get(task_name, EvalTask)(
                pred_dir, eval_dir, baseline_dir, self.max_instances
            )
...
def make_parser():
    ...
    parser.add_argument(
        "--baseline_model",
        type=str,
        help="Name of baseline model for head-to-head comparisons.",
    )
    parser.add_argument(
        "--max_instances",
        type=int,
