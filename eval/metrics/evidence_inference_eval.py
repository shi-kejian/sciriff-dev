...
class EvidenceInferenceEval:
    ...
    def __init__(self, threshold, model_name, score_type, n_samples=None):
        ...
        if score_type not in ['bertscore_p', "bertscore_f", 'bertscore_r', 'nli']:
            raise ValueError("incompatible score_type - must be 'bertscore_<p/r/f>' or 'nli'")
        if score_type == "nli" and "nli" not in model_name:
            raise ValueError("incompatible model_name - must be an nli model if score_type='nli'")
        self.model_name = model_name
        self.lang = "en-sci" if "sci" in model_name else "en"
        self.scores = {"by_entry": {}, "overall": {}}
        self.score_type = score_type
        self.bertscore_type = BERT_SCORE_TYPES[score_type] if "bertscore" in score_type else None

    @staticmethod
    def calculate_instance_scores(pred_matches, ref_matches, total_recall, total_precision, total_f1):
        precision = sum(pred_matches) / len(pred_matches) if len(
            pred_matches) > 0 else 0  # ratio of predictions that had at least 1 matching reference
        recall = sum(ref_matches) / len(ref_matches) if len(
            ref_matches) > 0 else 0  # ratio of references that were matched to at least 1 prediction
    ...
    def format_tuples_as_sentences(instance):
        predictions = [
            ...
            for e in instance['prediction'] if len(e) == 5]
        references = [
            f"Intervention: '{e[0]}', Comparator: '{e[1]}', Outcome: '{e[2]}', Effect: '{e[3]}', Evidence: '{e[4]}'"
            for e in instance['reference'] if len(e) == 5]
    ...
    def log_entry_scores(self, entry_type, total_recall, total_precision, total_f1):
        self.scores['by_entry'][entry_type] = {"recall": np.mean(total_recall),
                                               "precision": np.mean(total_precision),
                                               "f1": np.mean(total_f1)}

    def log_overall_scores(self):
        self.scores['overall']['per_entry'] = {
            "recall": np.mean([item['recall'] for item in self.scores['by_entry'].values()]),
            "precision": np.mean([item['precision'] for item in self.scores['by_entry'].values()]),
    ...
    def evaluate_by_entry_type(self, instances, scorer, save_individual_scores):
        ...
        if save_individual_scores:
            scores_per_entry = pd.DataFrame(columns=['instance', 'entry_type', 'prediction', 'reference', 'score'])
        for i, entry_type in enumerate(ENTRY_TYPES):  # for each entry type
            ...
            for j, instance in instances.items():
                ...
                if len(instance['prediction']) == 0 or len(instance['reference']) == 0:
                    continue
                pred_matches, ref_matches = [0] * len(instance['prediction']), [0] * len(instance['reference'])
                ...
                for p, pred in enumerate(instance['prediction']):
                    for r, ref in enumerate(instance['reference']):
                        if len(pred) != 5 or len(ref) != 5 or pred[i] is None or ref[i] is None:
                            continue

                        score = self.get_score(instance_scores, pred[i], ref[i], scorer)
                        if save_individual_scores:
                            new_row = pd.DataFrame([(j, entry_type, pred[i], ref[i], score)],
                                                   columns=scores_per_entry.columns)
                            scores_per_entry = pd.concat([scores_per_entry, new_row], ignore_index=True)
                        if ("bertscore" in self.score_type and score >= self.threshold) or \
                                ("nli" in self.score_type and score['label'] == "ENTAILMENT" and score[
                                    'score'] >= self.threshold):
                            pred_matches[p], ref_matches[r] = 1, 1
                self.calculate_instance_scores(pred_matches, ref_matches, total_recall, total_precision, total_f1)
            self.log_entry_scores(entry_type, total_recall, total_precision, total_f1)
        self.log_overall_scores()

        if save_individual_scores:
            scores_per_entry.drop_duplicates(subset=['prediction', 'reference'], inplace=True)
            ...
            for p, pred in enumerate(predictions):
                for r, ref in enumerate(references):
                    ...
                    if ("bertscore" in self.score_type and score >= self.threshold) or \
                        ...
                        pred_matches[p], ref_matches[r] = 1, 1
            self.calculate_instance_scores(pred_matches, ref_matches, total_recall, total_precision, total_f1)
        self.scores['overall']['per_full_tuple'] = {"recall": np.mean(total_recall),
                                                    "precision": np.mean(total_precision),
                                                    "f1": np.mean(total_f1)}

    def evaluate(self, instances, evaluate_by_full_tuple=True, individual_score_file=None):
        """
        ...
        if self.n_samples is not None:
            random.seed(42)
            chosen_samples = sorted(random.sample(range(len(instances)), self.n_samples))
            instances = {key: value for key, value in instances.items() if key in chosen_samples}

        # Load model (for BERTscore / NLI)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        if "bertscore" in self.score_type:
            scorer = BERTScorer(model_type=self.model_name, lang=self.lang, device=device)
        else:
            scorer = pipeline("text-classification", model=self.model_name)

        # Calculate Scores
        scores_per_entry = self.evaluate_by_entry_type(instances, scorer, individual_score_file)
